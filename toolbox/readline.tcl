...
if {$::tcl_platform(os)=="Linux"} {
  ...
  proc prompt {{txt ""}} {
      ...
      set txt "[ESC]\[0;93m$prompt[ESC]\[1;36m$txt[ESC]\[0m"
  ...
  set waitTAB 0
  proc handleCompletion {command_collection} {
      global env waitTAB
      set vars ""
      set cmds ""
      set execs ""
	  ...
	  set lastchar [string index $word $wordend]
      if {$lastchar == " "} {
		  catch "ls -p" res
		  regsub "\n" $res "  " res 
          if {$res != ""} {
      ...
      if {"$word" == "$shortest"} {
              ...
              foreach {match format} {
                  ...
                  if {[llength [set $match]]} {
                      ...
                      foreach x [set $match] {
						  ...
						  lappend cmd_list [file tail $x]
                      }
                      append temp "[ESC]\[0m"
                  }
              }
              print "\n$temp\n"
			  if {[lsearch $cmd_list $word]!=-1} {
			  	  set waitTAB 1
		      }
          } else {
          ...
          if {$shortest != ""} {
			  ...
			  if {[llength $maybe] == 1 && [string index $shortest end]!="/"} {
                 append env(CMDLINE) " "
     		     incr env(CMDLINE_CURSOR)
		      } 
  ...
  proc tclline {} {
      ...
      while {$keybuffer != ""} {
          ...
          if {[string is print $char]} {
              ...
              handleCompletion $command_collection
          } elseif {$char == "\n" || $char == "\r"} {
